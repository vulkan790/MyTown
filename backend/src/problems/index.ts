...
export async function ProblemsController (fastify: FastifyTypebox) {
  ...
  fastify.register(async (authFastify) => {
    ...
    authFastify.post('/:id/town', { schema: schema.solveProblemSchema }, solveProblem);
...
async function solveProblem (
  this: FastifyInstance,
  request: FastifyRequestTypeBox<schema.SolveProblemSchema>,
  reply: FastifyReplyTypeBox<schema.SolveProblemSchema>
) {
  const { id } = request.params;
  const { action } = request.body;

  const result = await this.problemService.solveProblem({
    id,
    status: action,
  }, request.user);
